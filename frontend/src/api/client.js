...
export function startInterview({ resumeText, targetPosition = '', targetJobId = null, interviewMode = '技术面' }) {
  return mockFallback(
    ...
    () => {
      const posLabel = targetPosition || '目标岗位';
      const modeQuestion = {
        '技术面': `请结合项目经历说明你为什么适合${posLabel}？并展开一个你最有把握的技术点。`,
        'HR面': `请用 2 分钟做自我介绍，并说明你为什么选择${posLabel}这个岗位。`,
        '压力面': `如果这次面试你没有被录用，你认为最可能的原因是什么？请结合${posLabel}岗位要求分析。`,
        '反馈教练': `刚才的回答里，哪一部分你觉得自己表达得最不清楚？`,
      };
      return {
        session_id: `mock-${Date.now()}`,
        question: modeQuestion[interviewMode] || modeQuestion['技术面'],
        interview_mode: interviewMode,
        position_bucket: targetPosition || '',
...
export function auditResume({ resumeText, targetPosition = '', resumeId = null }) {
  return mockFallback(
    ...
    () => {
      ...
      return {
        ...
        risk_flags: [
          '模糊表达：存在空泛/套话（如 负责过、相关经验）',
          `关键词缺失：建议补充 ${normalized ? normalized + ' 相关技能关键词' : '岗位关键词'}`,
        ],
        suggestions: [
          '增加量化成果（效率提升%、用户增长等），增强说服力。',
          '用 STAR 描述项目：明确你的角色、行动与可量化结果。',
        ],
        missing_keywords: ['量化结果', '个人贡献'],
        risk_level: '中',
        rule_score: 68,
        llm_score: null,
        detected_fields: {
          email: true, phone: true, education: true, experience: true,
          projects: true, skills: true, portfolio: false,
        },
