...
from app.schemas.job import JobAuditRequest, JobCreate, JobDetailOut, JobPostingOut
...
def _to_detail(job: JobPosting) -> dict:
    return {
        "id": job.id,
        "title": job.title,
        "company": job.company,
...
async def get_job_detail(
    ...
    """Get full job detail including responsibilities, requirements, etc."""
    job = db.get(JobPosting, job_id)
    if not job:
        raise AppException(404, 40405, "job not found")
    return success_response(_to_detail(job))
