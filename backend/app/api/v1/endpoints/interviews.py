...
def report(
    ...
    if session.status != "completed":
        raise AppException(400, 40001, "请先结束面试再查看报告")

    state = _load_agent_state(session)
    summary = _session_summary(session)
    summary["messages"] = _visible_messages(session)
    agent_report = state.get("completed_report")

    if isinstance(agent_report, dict):
        # 重新计算并修复旧报告分数不一致、维度未归一化等问题
        agent_report = _recompute_report_scores(agent_report)
        summary["score"] = agent_report.get("overall_score", summary.get("score", 0))
        summary["feedback"] = {
            "overall": agent_report.get("summary", ""),
            "strengths": agent_report.get("strengths", []),
            "weaknesses": agent_report.get("weaknesses", []),
        }
        summary["dimension_scores"] = agent_report.get("dimension_averages", {})
        summary["jobTitle"] = summary.get("job_title") or agent_report.get("target_position", "")

    summary["agent_report"] = agent_report
